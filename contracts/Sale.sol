...
contract Sale is Ownable, ReentrancyGuard {
  ...
  event eventBuy(
    ...
    uint256 indexed amountCurrency,
    uint256 indexed amountOur
  );
  ...
  function buyToken(uint256 _id, uint256 _amount) public nonReentrant {
    ...
    uint256 segmentCount = tokens[_id].increaseEvery -
      (tokens[_id].buyAmount % tokens[_id].increaseEvery);
